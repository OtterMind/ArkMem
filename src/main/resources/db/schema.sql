...
create table if not exists arkmem_memories (
  id uuid primary key,
...
create table if not exists arkmem_memory_history (
  id uuid primary key,
...
alter table arkmem_memories
  add column if not exists user_id text generated always as (metadata ->> 'user_id') stored;

alter table arkmem_memories
  add column if not exists agent_id text generated always as (metadata ->> 'agent_id') stored;

alter table arkmem_memories
  add column if not exists run_id text generated always as (metadata ->> 'run_id') stored;

alter table arkmem_memories
  add column if not exists keyword_search_vector tsvector generated always as (
    to_tsvector('simple'::regconfig, coalesce(memory, ''))
  ) stored;
...
drop index if exists arkmem_memories_active_user_idx;
drop index if exists arkmem_memories_active_agent_idx;
drop index if exists arkmem_memories_active_run_idx;

create index if not exists arkmem_memories_active_created_at_idx
  on arkmem_memories (created_at desc)
  where deleted_at is null;

create index if not exists arkmem_memories_active_user_idx
  on arkmem_memories (user_id, created_at desc)
  where deleted_at is null and user_id is not null;

create index if not exists arkmem_memories_active_exact_user_idx
  on arkmem_memories (user_id, created_at desc)
  where deleted_at is null and user_id is not null and agent_id is null and run_id is null;

create index if not exists arkmem_memories_active_agent_idx
  on arkmem_memories (agent_id, created_at desc)
  where deleted_at is null and agent_id is not null;

create index if not exists arkmem_memories_active_user_agent_idx
...
create index if not exists arkmem_memories_active_run_idx
  on arkmem_memories (run_id, created_at desc)
  where deleted_at is null and run_id is not null;

create index if not exists arkmem_memories_active_user_agent_run_idx
...
create index if not exists arkmem_memories_active_metadata_gin_idx
  on arkmem_memories using gin (metadata jsonb_path_ops)
  where deleted_at is null;

create index if not exists arkmem_memories_active_keyword_search_idx
  on arkmem_memories using gin (keyword_search_vector)
  where deleted_at is null;

create index if not exists arkmem_memory_history_memory_id_created_at_idx
