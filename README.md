...
| `ARKMEM_LLM_API_KEY` | LLM Provider API key |
...
| `ARKMEM_EMBEDDING_API_KEY` | Embedding Provider API key |
...
curl 'http://localhost:8080/exact/memories?user_id=user-1&agent_id=chat-agent'
curl 'http://localhost:8080/memories/query?user_id=user-1&agent_id=chat-agent&source=manual-curl&limit=10&offset=0'
```
...
| `aliyun-bailian` | 使用 DashScope compatible mode |
| `auto` | 优先使用 `OPENAI_API_KEY` 或 `DASHSCOPE_API_KEY`，不存在时回退到本地模式 |

远程 Provider 配置示例：

```bash
export ARKMEM_LLM_PROVIDER="openai-compatible"
export ARKMEM_EMBEDDING_PROVIDER="openai-compatible"
export ARKMEM_LLM_API_KEY="your-api-key"
export ARKMEM_EMBEDDING_API_KEY="your-api-key"
export ARKMEM_LLM_BASE_URL="https://example.com/v1"
export ARKMEM_EMBEDDING_BASE_URL="https://example.com/v1"
export ARKMEM_LLM_MODEL="gpt-4.1-nano-2025-04-14"
export ARKMEM_EMBEDDING_MODEL="text-embedding-3-small"
```
